...
export function SizesManager({ sizes }: Props) {
  ...
  async function runCreate(formData: FormData) {
    setFormError(null);
    const r = await createSize(formData);
    if (r.error) {
    ...
    (document.getElementById("create-size-form") as HTMLFormElement)?.reset();
  }

  async function runUpdate(formData: FormData) {
    setRowError(null);
    const r = await updateSize(formData);
    if (r.error) {
      setRowError(r.error);
      return;
    }
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <form id="create-size-form" action={runCreate} className="mt-4 space-y-4">
      ...
      <div className="overflow-hidden rounded-xl border border-[#245236]/20 bg-white shadow-sm">
        {sizes.length === 0 ? (
          <p className="p-8 text-center text-sm text-zinc-500">No sizes yet.</p>
        ) : (
